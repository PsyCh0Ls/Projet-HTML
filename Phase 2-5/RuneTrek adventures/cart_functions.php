        ...
        foreach ($trip['stages'] as $stage_index => $stage) {
            if (isset($stage['options'])) {
                foreach ($stage['options'] as $option_index => $option) {
                    ...
                    if (isset($options[$stage['id']][$option['name']])) {
                        $option_data = $options[$stage['id']][$option['name']];
                        $selected_value = $option_data['value'];
                        $person_count = isset($option_data['persons']) ? (int)$option_data['persons'] : 1;
                        
                        // Chercher le prix de l'option sélectionnée
                        if (isset($option['values'])) {
                            foreach ($option['values'] as $value) {
                                if ($value['value'] == $selected_value) {
                                    // Ajouter le prix de cette option au total en tenant compte du nombre de personnes
                                    $total_price += $value['price'] * $person_count;
                                    break;
                                }
                            }
                        }
